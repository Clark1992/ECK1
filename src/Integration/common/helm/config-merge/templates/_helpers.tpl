...
kubectl get configmaps -l config-type=integration-config -o json > "$TMP"

COUNT=$(jq '.items | length' "$TMP")
echo "[config-merge] Found $COUNT ConfigMaps"

if [ "$COUNT" -eq 0 ]; then
  echo '{ "IntegrationConfig": {} }' > "$OUTPUT"
  exit 0
fi
...
echo "[config-merge] Merging all entries into IntegrationConfig..."

echo "$PARSED" \
  | jq -s 'reduce .[] as $item ({}; . * $item) | { IntegrationConfig: . }' \
  > "$OUTPUT"
