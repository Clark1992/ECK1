...
$baseDir = "src/Integration/ECK1.VersionTracker"
$dockerfilePath = "$baseDir/Dockerfile"
$imageName = "version-tracker"
$imageTag = "dev"
...
helm upgrade --install $appReleaseName $baseDir\Deploy\$serviceChartPath `
    ...
    -f $baseDir\Deploy\$serviceChartPath\values.local.yaml `
    -f $baseDir\Deploy\$serviceChartPath\values.secrets.yaml
